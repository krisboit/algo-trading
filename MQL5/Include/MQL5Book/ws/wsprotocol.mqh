...
protected:
   ...
   void processControlFrame(IWebSocketFrame *frame)
   {
      ...
      case WS_FRAME_OPCODE::WS_PONG_FRAME:
...
public:
   ...
   bool handshake(const string url, const string host, const string origin, const string custom = NULL) override
   {
      ...
      // Extract just the path from the URL (e.g., "/mt5" from "ws://example.com/mt5")
      string urlParts[];
      URL::parse(url, urlParts);
      string path = urlParts[URL_PATH];
      if(path == NULL || path == "") path = "/";
  
      WsTools::push(headers, "GET", path + " HTTP/1.1");
